...
void main(){
	...
	float dproduct = dot(normal, -normalize(worldPos0 - fs_Pos));
	//Fade with distance, light becomes less bright as it reaches the end of its cone.
	float maxDistLight = 1/pow(height, 2);
	float distanceFade = clamp((brightness/pow(length(worldPos0 - fs_Pos), 2)-maxDistLight)*height, 0, 1);
	//The shadow calculation, how much of it is in the light
	float shadow = calcShadow(proj_coord);
	
	float bright = 0.2126*color.r + 0.7152*color.g + 0.0722*color.g;
	bright = pow(bright, 0.1);
	//Multiply everything together to get the final color (also threw in a 3 so it looked bright enough)
	gl_FragColor = vec4(texture2D(flashlightTex, proj_coord.st).rgb*color.rgb*(1-bright)*20*test*dproduct*distanceFade*shadow, 1);
}