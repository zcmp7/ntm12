...
void main(){
	...
	vec3 viewDirection = normalize(world);
	hit intersect = intersectSpotlight(vec3(0), viewDirection, pos, direction, cosAngle, height);
	//vec3[2] intersect = intersectCone(vec3(0), viewDirection, pos, vec3(0, 1, 0), cos(20*0.0174533), 10);
	if(intersect.hitType >= 0){
		vec3 ray = intersect.pos2-intersect.pos1;
		float len = length(ray);
		vec3 rayDir = ray/len;
		
		vec4 texPos1 = flashlight_ViewProjectionMatrix * vec4(intersect.pos1, 1);
		vec4 texPos2 = flashlight_ViewProjectionMatrix * vec4(intersect.pos2, 1);
		...
		for(int i = 0; i < NUM_STEPS; i ++){
			...
			vec2 fs_texCoord = ((currentTexPos.xyz/currentTexPos.w)*0.5 + 0.5).xy;
			accum += scattering;
			currentPos += stepVec;
		}
