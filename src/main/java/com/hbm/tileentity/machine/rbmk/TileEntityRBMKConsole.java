...
public class TileEntityRBMKConsole extends TileEntityMachineBase implements IControlReceiver, ITickable, SimpleComponent {
		...
		for(int i = -7; i <= 7; i++) {
			for(int j = -7; j <= 7; j++) {
				...
				if(te instanceof TileEntityRBMKBase) {
					...
					columns[index].data.setDouble("maxWater", rbmk.maxWater);
					columns[index].data.setDouble("steam", rbmk.steam);
					columns[index].data.setDouble("maxSteam", rbmk.maxSteam);
	...
	@Callback(doc = "func(i:str): retrieves all column data for given index")
