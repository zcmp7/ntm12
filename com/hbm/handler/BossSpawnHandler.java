	...
	public static void rollTheDice(World world) {
		...
		if(world.getTotalWorldTime() % delay == 0) {
			...
			if(world.rand.nextInt(3) == 0 && !world.playerEntities.isEmpty() && world.provider.isSurfaceWorld()) {	//33% chance only if there is a player online
				...
				if(ContaminationUtil.getRads(player) >= 50 && world.getHeight((int)player.posX, (int)player.posZ) > player.posY + 3) {	//if the player has more than 50 RAD and is underground

					double spawnX = player.posX + world.rand.nextGaussian() * 20;
					double spawnZ = player.posZ + world.rand.nextGaussian() * 20;
					double spawnY = world.getHeight((int)player.posX, (int)player.posZ);
					...
					Result canSpawn = ForgeEventFactory.canEntitySpawn(maskman, world, (float)spawnX, (float)spawnY, (float)spawnZ, null);

					if (canSpawn == Result.ALLOW || canSpawn == Result.DEFAULT) {

						world.spawnEntity(maskman);
						ForgeEventFactory.doSpecialSpawn(maskman, world, (float)spawnX, (float)spawnY, (float)spawnZ, null);
					}
				}
			}
		}
