...
public class GUIMachineRadar extends GuiInfoContainer {
	...
	public void drawScreen(int mouseX, int mouseY, float f) {
		...
		if(!diFurnace.nearbyMissiles.isEmpty()) {
			for(int[] m : diFurnace.nearbyMissiles) {
				int x = guiLeft + (int)((m[0] - diFurnace.getPos().getX()) / ((double)MainRegistry.radarRange * 2 + 1) * (200D - 8D)) + 108;
				int z = guiTop + (int)((m[1] - diFurnace.getPos().getZ()) / ((double)MainRegistry.radarRange * 2 + 1) * (200D - 8D)) + 117;
				...
				if(mouseX + 4 > x && mouseX - 4 < x && 
					...
					if(m[2] == 0)
						...
						text = new String[] { "Custom Missile", m[0] + " / " + m[1], "Alt.: " + m[3] };
					/*else if(m[2] == 7)
	...
	protected void drawGuiContainerBackgroundLayer(float p_146976_1_, int p_146976_2_, int p_146976_3_) {
		...
		if(!diFurnace.nearbyMissiles.isEmpty()) {
			for(int[] m : diFurnace.nearbyMissiles) {
				int x = (int)((m[0] - diFurnace.getPos().getX()) / ((double)MainRegistry.radarRange * 2 + 1) * (200D - 8D)) - 4;
				int z = (int)((m[1] - diFurnace.getPos().getZ()) / ((double)MainRegistry.radarRange * 2 + 1) * (200D - 8D)) - 4;
				int t = m[2];
