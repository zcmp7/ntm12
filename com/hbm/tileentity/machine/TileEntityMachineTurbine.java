...
public class TileEntityMachineTurbine extends TileEntity implements ITickable, ISource, ITankPacketAcceptor, IFluidHandler, IClientRequestUpdator {
	...
	public void update() {
		...
		if(!world.isRemote)
		{
			...
			if(needsTankTypeUpdate){
				PacketDispatcher.wrapper.sendToAll(new FluidTypePacketTest(pos.getX(), pos.getY(), pos.getZ(), tankTypes));
				needsTankTypeUpdate = false;
			}
	...
	private void detectAndSendChanges() {
		...
		if(!FFUtils.areTanksEqual(detectTanks[0], tanks[0]) || clientRequestUpdate){
			mark = true;
			needsUpdate = true;
			detectTanks[0] = FFUtils.copyTank(tanks[0]);
		}
		if(!FFUtils.areTanksEqual(detectTanks[1], tanks[1]) || clientRequestUpdate){
			mark = true;
			needsUpdate = true;
			detectTanks[1] = FFUtils.copyTank(tanks[1]);
		}
		if(detectFluids[0] != tankTypes[0] || clientRequestUpdate){
			mark = true;
			needsTankTypeUpdate = true;
			detectFluids[0] = tankTypes[0];
		}
		if(detectFluids[1] != tankTypes[1] || clientRequestUpdate){
			mark = true;
			needsTankTypeUpdate = true;
			detectFluids[1] = tankTypes[1];
		}
		if(mark && !clientRequestUpdate)
			markDirty();
